...
class Script(DefaultScript):
    slug = 'simple_features_generator'
    ...
    def build_seq_to_seq_graph(self):
        ...
        encoder = GRU(32, return_sequences=True, return_state=True, dropout=0.5, recurrent_dropout=0.5)
        decoder = GRU(32, return_sequences=True, return_state=True, dropout=0.5, recurrent_dropout=0.5)

        masking = keras.layers.Masking(mask_value=1)

        sentence_1_embedded = embedding_layer(masking(sentence_1))
        sentence_2_embedded = embedding_layer(masking(sentence_2))
        sentence_3_embedded = embedding_layer(masking(sentence_3))
        sentence_4_embedded = embedding_layer(masking(sentence_4))
        ending_1_embedded = embedding_layer(masking(ending_1))
