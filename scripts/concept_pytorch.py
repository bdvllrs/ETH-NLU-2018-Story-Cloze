...
class Script(DefaultScript):
            ...
            for phase in ['train','test']:
                ...
                if phase == 'train':
                    for num, batch in enumerate(generator_training):
                        print('NUM BATCH')
                        print(num)
                        main_loss_total, loss_auto_debut, loss_auto_fin, loss_cross_debut, loss_cross_fin=Seq2SEq_main_model.train_all(batch)
                        ...
                        if num % self.config.plot_every == self.config.plot_every - 1:
                            ...
                            Seq2SEq_main_model.time_since(start, (num+1) / (90000/32)), (num+1), (num+1) / (90000/32) * 100,
                        ...
                        try:
                            if compteur_val==3:
                                ...
                                for num, batch in enumerate(generator_dev):
                                    if num<21:
                                        all_histoire_debut_embedding = Variable(torch.FloatTensor(batch[0])).transpose(0,
                                                                                                                      1).cuda()
                                        all_histoire_fin_embedding1 = Variable(torch.FloatTensor(batch[1])).transpose(0,
                                                                                                                     1).cuda()
                                        all_histoire_fin_embedding2 = Variable(torch.FloatTensor(batch[2])).transpose(0,
                                                                                                                     1).cuda()
                                        labels = Variable(torch.FloatTensor(batch[3])).cuda()
                    ...
                    total=0
                    for num, batch in enumerate(generator_dev):
                        all_histoire_debut_embedding = Variable(torch.FloatTensor(batch[0])).transpose(0,1).cuda()
                        all_histoire_fin_embedding1 = Variable(torch.FloatTensor(batch[1])).transpose(0,1).cuda()
                        all_histoire_fin_embedding2 = Variable(torch.FloatTensor(batch[2])).transpose(0,1).cuda()
                        labels=Variable(torch.LongTensor(batch[3]))
                        end=Seq2SEq_main_model.evaluate(Seq2SEq_main_model.encoder_source, Seq2SEq_main_model.decoder_target,
                                                            all_histoire_debut_embedding, Seq2SEq_main_model.input_length_debut)
                        debut1= Seq2SEq_main_model.evaluate(Seq2SEq_main_model.encoder_source, Seq2SEq_main_model.decoder_target,
                                                            all_histoire_fin_embedding1, Seq2SEq_main_model.input_length_fin)
                        debut2= Seq2SEq_main_model.evaluate(Seq2SEq_main_model.encoder_source, Seq2SEq_main_model.decoder_target,
                                                            all_histoire_fin_embedding2, Seq2SEq_main_model.input_length_fin)
                        preds=self.get_predict(end,debut1,debut2,all_histoire_debut_embedding.transpose(0,1),all_histoire_fin_embedding1.transpose(0,1),all_histoire_fin_embedding2.transpose(0,1))
    ...
    def get_predict(self, end, debut1, debut2, all_histoire_debut_embedding, all_histoire_fin_embedding1,
        ...
        debut1=debut1
        debut2 = debut2
        all_histoire_debut_embedding=all_histoire_debut_embedding
        end=end
        all_histoire_fin_embedding1=all_histoire_fin_embedding1
        all_histoire_fin_embedding2 = all_histoire_fin_embedding2
        for num_batch, batch in enumerate(end):
            for num_sent, sent in enumerate(batch):
                produit=torch.dot(sent.float(),all_histoire_fin_embedding1[num_batch][num_sent].float())/ (torch.norm(sent.float()) * torch.norm(all_histoire_fin_embedding1[num_batch][num_sent].float()))
                ...
                produit = torch.dot( debut1[num_batch][num_sent].float(), all_histoire_debut_embedding[num_batch][num_sent].float()) / (
                    torch.norm(debut1[num_batch][num_sent].float()) * torch.norm(all_histoire_debut_embedding[num_batch][num_sent].float()))
                semblable_debut1.append(produit.cpu().data[0])
                produit = torch.dot(debut2[num_batch][num_sent].float(), all_histoire_debut_embedding[num_batch][num_sent].float()) / (
