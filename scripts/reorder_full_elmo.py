...
def output_fn(data):
    ...
    for b in batch:
        if random.random() > 0.5:
            ...
            to_invert = random.randint(0, 3)
            sentence1 = sentence2 = sentence3 = sentence4 = " ".join(b[4])
            if to_invert != 0:
                sentence1 = " ".join(b[0])
            if to_invert != 1:
                sentence2 = " ".join(b[1])
            ...
            sentences4.append(sentence4)
            sentences5.append(" ".join(b[to_invert]))
            label.append(0)
...
class Script(DefaultScript):
    ...
    def train(self):
        ...
        # Callbacks
        tensorboard = keras.callbacks.TensorBoard(log_dir='./logs/' + timestamp + '-reorder-full-elmo/',
                                                  histogram_freq=0,
        ...
        model_path += '-reorder-full-elmo_checkpoint_epoch-{epoch:02d}.hdf5'
